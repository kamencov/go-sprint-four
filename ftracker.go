...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	heightCmInM := height / cmInM
	speedkmhInMsec := meanSpeed(action, duration) * kmhInMsec
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedkmhInMsec, 2)/heightCmInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
